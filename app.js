(async function () {
  ...
  const TOPOJSON_CDN_URL = "https://cdn.jsdelivr.net/npm/topojson-client@3/dist/topojson-client.min.js";
  ...
  function loadScriptFromCdn(src) {
    return new Promise((resolve, reject) => {
  ...
  async function ensureMapLibraries() {
    if (!window.d3) await loadScriptFromCdn(D3_CDN_URL);
    if (!window.topojson) await loadScriptFromCdn(TOPOJSON_CDN_URL);
  }
  ...
  async function renderWorldMap() {
    try {
      ...
      console.warn("Map libraries could not load from CDN.", error);
    }
  ...
  function uploadSidebarBackground(file) {
    ...
    reader.addEventListener("load", async () => {
      ...
      if (!saved) {
        alert("This browser view could not save the sidebar background. Try a smaller image.");
      }
      applySidebarBackground(region);
    });
    ...
    region.practices.forEach((practice) => {
      ...
      card.innerHTML = `
        ...
        <div class="practice-detail">
          ...
          ${IS_ADMIN ? `<div class="card-actions">
  ...
  function imagePanel(practice) {
    ...
    return `
      <div class="practice-image-panel">
        <div class="practice-image-frame ${imageSrc ? "has-image" : ""}">
          ${imageMarkup}
        </div>
        ${IS_ADMIN ? `<label class="image-url-label" for="${urlInputId}">Image URL</label>
        <input
  ...
  function uploadPracticeImage(practiceId, file) {
    ...
    reader.addEventListener("load", async () => {
      ...
      const saved = await saveData();
      if (!saved) {
        alert("This browser view could not save the image. Try a smaller image.");
      }
      renderPractices();
  ...
  if (els.resetDataButton) els.resetDataButton.addEventListener("click", resetData);
